...
class Simulation:
    ...
    """
    def __init__(self, florida_landfall_rate: float, florida_mean: float, florida_stddev: float, gulf_landfall_rate: float, gulf_mean: float, gulf_stddev: float):        
    ...
    def simulate(self, years: int, use_CLT: bool = False) -> float:
        """
        ...
        """
        if (years <= 0):
            logger.info(f"Simulation for no time.")
            return 0
