...
class HurricaneSimulator:
    ...
    """
    def __init__(self, rate: float, mean: float, stddev: float):
        if (rate <= 0):
        ...
        self.mean = mean
        self.stddev = stddev
        logger.info(f"HurricaneSimulation initialized with rate={rate}, mean={mean}, stddev={stddev}.")
    
    def simulate(self, years: int, batch_size: int = 1000, use_CLT: bool = False) -> float:
        """
        ...
        if use_CLT:
            logger.info(f"Simulating using CLT.")
